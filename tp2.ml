...
class gestionnaireReseau 
  ...
  object(self)
	...
	?(date = Some(date_actuelle ())) l_num =
      ...
      (* Traitement correspondant aux  préconditions  *)
			(*if (List.length(self#trouver_voyages_sur_la_ligne ~date:(date) l_num)<=0)
																						 then raise(Erreur "Date invalide");*)
	
			if not(H.mem lignes l_num) then raise (Erreur "Ligne invalide"); 
      (* Traitement correspondant à la fonction *)
			
      let idDesVoyages = self#trouver_voyages_sur_la_ligne ~date:(date) l_num in
			
			let	tousLesVoyages =
				H.fold (fun k v acc -> if(L.exists (fun x->x = k) idDesVoyages) then
						v::acc else acc) voyages [] in

			
			let th = H.create 10000 in
			
			let listeAvecType = L.iter (fun (v:voyage)->H.add th 
							v#get_direction v#get_stations_des_arrets) tousLesVoyages in
			
			let max' x y = if (L.length x) > (L.length y) then x else y in
			L.map (fun dest -> (match dest with
			| AllerVers d -> d
			| RetourVers d ->d),
			 top_liste max' (H.find_all th dest)) (cles th) 
    ...
    method (*private*) prochain_arret 
      ...
      (* Traitement correspondant aux préconditions *)
			...
			if (L.length (self#trouver_voyages_par_date() ~date:(date)) = 0)then 
																									raise (Erreur "Mauvaise date");
																	
			if not(H.mem lignes l_num) then raise (Erreur "Ligne invalide"); 
			
			if not(H.mem stations sid) then raise (Erreur "La station n'existe pas");
		
			if heure < 0 then raise (Erreur "Heure négative");
			
			if not (self#ligne_passe_par_station ~date:(Some date) l_num sid) then
				raise (Erreur "La ligne n'existe pas entre les stations");
			
			 (* Traitement correspondant à la fonction *)
			let idDesVoyages = 
				self#trouver_voyages_sur_la_ligne l_num ~date:(Some date) in
			
			let	tousLesVoyages =
				H.fold (fun k v acc -> if(L.exists (fun x->x = k) idDesVoyages) then
						v::acc else acc) voyages [] in
						
			let lesVoyagesBonneDirection = L.filter (fun (x:voyage) ->  	
			(if (x#get_direction = direction) then true else false))
			tousLesVoyages in
			
			
			if ((L.length lesVoyagesBonneDirection) = 0) then
				raise (Erreur "Mauvaise direction") else
			
			let listeFiltree = L.filter 
                	(fun (x:voyage)->
									if(L.exists (fun x-> x = sid) x#get_stations_des_arrets) 
                	then true else false) lesVoyagesBonneDirection in

			let tousLesArrets = 
				L.map (fun (x:voyage)-> x#get_arret_de_station sid) listeFiltree in
			
			let min' (x:arret) (y:arret) = 
				if (x#get_arrivee) = (y#get_arrivee) then 0 else if
			(x#get_arrivee) > (y#get_arrivee) then 1 else -1 in
			
		  let arretsTrier = L.sort min' tousLesArrets in
			
      let arretsPossible = L.filter (fun (x:arret) -> 
				if ((x#get_arrivee - heure) <= 0) then false else true) arretsTrier in 
			
			if L.length arretsPossible = 0 then
				raise (Erreur "Il n y a plus d'arret") else
    ...
    method (*private*) prochaines_lignes_entre 
      ...
      (* Traitement correspondant aux préconditions *)
			...
			if not(H.mem stations sid_dep) then 
				raise (Erreur "La station de depart n'existe pas");
				
			if not(H.mem stations sid_dest) then 
				raise (Erreur "La station destination n'existe pas");
		
			if heure < 0 then raise (Erreur "Heure négative");	
																																																																							
      (* Traitement correspondant à la fonction *)
			
      let lignePasseParDep = self#lister_lignes_passantes sid_dep in
			let lignePasseParDest = self#lister_lignes_passantes sid_dest in
			
			let lesNumsLigne = L.filter 
			(fun x -> self#ligne_passe_par_station ~date:(Some date)  x sid_dest)
																												 lignePasseParDep in		
			

			let tupleLigneVoyageId = 
				 L.map 
				(fun x -> x,(self#trouver_voyages_sur_la_ligne ~date:(Some date) x))
																															lesNumsLigne in 		
			
			let tupleLigneVoyage =
				 L.map (fun x -> (fst x),(L.map (fun y -> H.find voyages y) (snd x)))
																												tupleLigneVoyageId in
			
			let tupleLigneVoyageDispo = 
				L.map (fun (x,y) -> 
					(x,L.filter (fun x ->
						 if(x#get_heure_fin < heure)then false else true) y))
																							 tupleLigneVoyage in
			
			
			let tupleLigneVoyageNonVide =
				 L.filter (fun (x,y) ->
					 (if L.length y = 0 then false else true)) tupleLigneVoyageDispo in 
			

			let liste  = L.map (fun (x,y)-> (x,L.filter 
                	(fun (x:voyage)->
									if(L.exists (fun x-> x = sid_dep) x#get_stations_des_arrets) 
                	then true else false) y)) tupleLigneVoyageNonVide in
				
			
			L.map (fun (x,y) ->
				 x, (self#prochain_arret ~date:(date)
															 ~heure:(heure)
															 (L.hd y)#get_direction x sid_dep)#get_arrivee,
			(self#prochain_arret ~date:(date)
													 ~heure:(heure)
													 (L.hd y)#get_direction x sid_dest)#get_arrivee)liste
  ...
  method (*private*) maj_etiquette_arete 
    ...
    (* Traitement correspondant aux préconditions *)
		if heure <= 0 then raise (Erreur "Heure négative");
    if not (H.mem stations sid1) then raise (Erreur "Station 1 inexistante ");
		if not (H.mem stations sid2) then raise (Erreur "Station 2 inexistante ");
...
(*
  ...
  method paires_stations_possibles 
      ?(rmax = distance_max_pieds) coord1 coord2 =
    (* Traitement correspondant aux préconditions *)
    (* Traitement correspondant à la fonction *)
    ...
    (* ----------------------------------------------------------------------- *)
  method plus_court_chemin sid_dep sid_dest = 
    (* Traitement correspondant aux préconditions *)
    (* Traitement correspondant à la fonction *)
  ...
  method trouver_trajet_optimal 
      ?(date = date_actuelle ())
      ?(heure = heure_actuelle ())
      ?(rmax = marche_max_pieds) coord1 coord2 =
    (* Traitement correspondant aux préconditions *)
    (* Traitement correspondant à la fonction *)
